...
import com.customdataindexr.elasticsearch.client.bulk.action.BulkProcessorLoggerLister;
...
public class ScrollBasedReIndexAction {
	...
	public ReindexingResponse reindex(ReIndexingRequest reIndexingRequestrequest) {
		...
		org.elasticsearch.action.search.SearchResponse searchResponse =  esClient.getClient()
				...
				.setTypes(reIndexingRequestrequest.getTypes())
				.setQuery(reIndexingRequestrequest.getBody())
				.setSearchType(SearchType.SCAN)
				.setScroll(new Scroll(reIndexingRequestrequest.getKeepAlive())).setSize(reIndexingRequestrequest.getSize()).execute().actionGet();
